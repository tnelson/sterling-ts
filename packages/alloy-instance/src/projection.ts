...
export function applyProjections(instance: AlloyInstance, atomIds: string[]): AlloyInstance {
  ...
  atomIds.forEach(atomId => {
    ...
    if (!projections[topType]) {
      ...
      throw new Error(`Cannot project ${atomId} and ${projections[topType]}. Both are of type ${topType}`);
    }
  });
...
function projectTypes(instance: AlloyInstance, projections: Record<string, string>): Record<string, AlloyType> {
  ...
  for (const typeId in instance.types) {
    ...
    const isProjected = projectedTypes.some(projectedType => typeIsOfType(instance, type, projectedType));
    types[typeId] = {
      _: 'type',
      id: type.id,
      types: type.types,
...
function projectRelations(instance: AlloyInstance, projections: Record<string, string>): Record<string, AlloyRelation> {
  ...
  for (const relationId in instance.relations) {
    ...
    const isProjected = relation.types
      .some(relationType => projectedTypes
        .some(projectedType => typeIsOfType(instance, relationType, projectedType)));
...
function getProjectedTypeIndices (instance: AlloyInstance, types: string[], projectedTypes: string[]): number[] {
  const indices: number[] = [];
  types.forEach((type, index) => {
    if (projectedTypes.some(projectedType => typeIsOfType(instance, type, projectedType))) {
      indices.push(index);
    }
  });
  return indices;
...
function projectTuples(tuples: AlloyTuple[], projectedIndices: number[], projectedAtoms: string[]): AlloyTuple[] {
  return tuples
    .filter(tuple => tuple.atoms.some(atom => projectedAtoms.includes(atom)))
    .map(tuple => {
      return {
        _: 'tuple',
